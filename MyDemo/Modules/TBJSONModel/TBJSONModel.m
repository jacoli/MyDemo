        ...
        while (class != [TBJSONModel class]) {
            ...
            for (unsigned int i = 0; i < propertyCount; i++) {
                ...
                if (propertyName) {
                    ...
                    TBJSONModelProperty *modelProperty = [TBJSONModelProperty propertyWithName:name typeString:typeString];
...
- (id)valueForProperty:(TBJSONModelProperty *)property withJSONValue:(id)value{
    ...
    if (value == nil || [value isKindOfClass:[NSNull class]]) {
        ...
        if (property->_valueType != TBClassPropertyTypeObject) {
            /*当属性为原始数据类型而对应的json dict中的value的类型为字符串对象的时候
            则对字符串进行相应的转换*/
            if ([value isKindOfClass:[NSString class]]) {
                if (property->_valueType == TBClassPropertyTypeInt ||
                    property->_valueType == TBClassPropertyTypeUnsignedInt||
                    property->_valueType == TBClassPropertyTypeShort||
                    property->_valueType == TBClassPropertyTypeUnsignedShort) {
                    resultValue = [NSNumber numberWithInt:[(NSString *)value intValue]];
                }
                if (property->_valueType == TBClassPropertyTypeLong ||
                          property->_valueType == TBClassPropertyTypeUnsignedLong ||
                          property->_valueType == TBClassPropertyTypeLongLong ||
                          property->_valueType == TBClassPropertyTypeUnsignedLongLong){
                    resultValue = [NSNumber numberWithLongLong:[(NSString *)value longLongValue]];
                }
                if (property->_valueType == TBClassPropertyTypeFloat) {
                    resultValue = [NSNumber numberWithFloat:[(NSString *)value floatValue]];
                }
                if (property->_valueType == TBClassPropertyTypeDouble) {
                    resultValue = [NSNumber numberWithDouble:[(NSString *)value doubleValue]];
                }
                if (property->_valueType == TBClassPropertyTypeChar) {
